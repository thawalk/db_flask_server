...
def add_book():
    data = request.json
    if("title",'asin','description','price','categories' not in data):
        print("THIS IS WRONG")
    title = data['title']
    asin = data['asin']
    ...
    print(title,asin,description,price,categories)
    if(not title or not asin or not description or not price or not categories):
