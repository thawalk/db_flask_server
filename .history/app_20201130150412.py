...
def sort_by_ratings():   #sort by increasing ratings,  decreasing rating
    try:
        ...
        if(rating_preference == 'increasing'): #means rating 1 will come out first
            mySQL_sort_query = """SELECT asin as asin,CAST(AVG(overall) AS CHAR) as rating FROM reviews.kindle_reviews GROUP BY asin ORDER BY AVG(overall) ASC limit 1;"""
        else: #means rating 5 will come out first
            mySQL_sort_query = """SELECT asin as asin,CAST(AVG(overall) AS CHAR) as rating FROM reviews.kindle_reviews GROUP BY asin ORDER BY AVG(overall) DESC limit 1;"""
        cur.execute(mySQL_sort_query)
        result_set = cur.fetchall()
        r = [dict((cur.description[i][0], value) \
               for i, value in enumerate(row)) for row in result_set]
        ...
        for data in r:
            ...
            try:
                ...
                metadata[0]['imUrl']=data['imUrl']
                final_result.append(metadata[0])
                print(metadata)
            except Exception as e:
                print(e)
