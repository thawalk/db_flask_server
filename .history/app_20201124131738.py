...
def sort_by_ratings():   #sort by increasing ratings,  decreasing rating
    try:
        ...
        else if if(rating_preference == 'increasing'): #means rating 5 will come out first
            mySQL_sort_query = """SELECT * FROM reviews.kindle_reviews ORDER BY overall DSC LIMIT 10;"""
        cur.execute(mySQL_sort_query)
        result_set = cur.fetchall()
        print(result_set)
        js = json.dumps(result_set)
        response = Response(js, status=200, mimetype='application/json')
        return response
