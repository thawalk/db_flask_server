...
@app.route('/search', methods=['GET'])  #now it only searches for TITLE
def search_book():
    data = request.json
    title = data["title"]
    try:
    ...
    # if(data["searchType"] == "AND"):
    #     try:
    #         author = data["author"] 
    #         title = data["title"]
    #         result = metadata_col.find({"author":author,"title":title})
    #         result_array = dumps(list(result))
    #         print(result_array)
    #         print(len(result_array))
    #         print("hello")
    #         if(len(result)==2):
    #             result = "No such item found"
    ...
    #             return response
    #     except:
    #         errMsg = "Please include both author and title."
